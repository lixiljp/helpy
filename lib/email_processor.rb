...
class EmailProcessor
  ...
  def process
    ...
    score += md.include?('shandong') ? 1 : 0
    score += md.include?('foshan') ? 1 : 0
    score += md.include?('sales') ? 1 : 0
    score += md.include?('dear') ? 1 : 0
    score += md.include?('products') ? 1 : 0
    score += md.include?('we are') ? 2 : 0
    score += md.include?('we can') ? 2 : 0
    score += md.include?('we hereby') ? 2 : 0
    score += md.include?('we strive') ? 2 : 0
    score += md.include?('we have') ? 2 : 0
    score += md.include?('we wish') ? 2 : 0
    score += md.include?('we could') ? 2 : 0
    score += md.include?('we look') ? 2 : 0
    score += md.include?('we understand') ? 2 : 0
    score += md.include?('choose us') ? 2 : 0
    score += md.include?('with us') ? 2 : 0
    score += md.include?('our service') ? 2 : 0
    score += md.include?('our company') ? 2 : 0
    score += md.include?('our email') ? 2 : 0
    score += md.include?('our new') ? 2 : 0
    score += md.include?('our product') ? 2 : 0
    score += md.include?('our speciality') ? 2 : 0
    score += md.include?('our expertise') ? 2 : 0
    score += md.include?('our price') ? 2 : 0
    score += md.include?('to provide') ? 1 : 0
    score += md.include?('services') ? 1 : 0
    score += md.include?('introduce') ? 1 : 0
    score += md.include?('for more information') ? 2 : 0
    score += md.include?('here are') ? 1 : 0
    score += md.include?('any interest') ? 2 : 0
    score += md.include?('welcome to') ? 2 : 0
    ...
    score += md.include?('cooperate') ? 1 : 0
    score += md.include?('opportunity') ? 1 : 0
    score += md.include?('potential') ? 1 : 0
    score += md.include?('appreciate') ? 1 : 0
    score += md.include?('leading') ? 1 : 0
    score += md.include?('manufact') ? 1 : 0
    score += md.include?('competitive') ? 1 : 0
    ...
    score += md.include?('公司') ? 3 : 0
    score += md.include?('产品') ? 3 : 0
    score += md.include?('您') ? 3 : 0
    score += md.include?('我们') ? 3 : 0
    score += md.include?('业务') ? 3 : 0
